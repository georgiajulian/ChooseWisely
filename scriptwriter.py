...
"""
    ...
    if game_state == "scene_1":
        system_prompt = """
You are an expert cybersecurity educator and screenwriter for "TrustNoOne".
...
JSON structure:
{
  ...
  "scene_dialogue": "The exact scam message text or narrator voiceover shown to the user (max 30 words). STRICT RULE: DO NOT use any emojis in this text, plain text only.",
  "tension_level": 3,
...
The victim is reacting emotionally and personally to the consequence of the user's choice. 
Example: "Oh no, my screen just turned red! I think I just downloaded a virus!" or "Wait, why is it asking for my credit card number? This feels wrong."
DO NOT use the narrator voice here. DO NOT describe the user's action in the 2nd person.

CRITICAL RULE FOR CHOICES: You MUST generate exactly 3 logical next steps based ONLY on the user's previous action.
...
{{
  ...
  "scene_dialogue": "The victim's first-person emotional reaction to what just happened (max 30 words). USE 'I' AND 'MY'. STRICT RULE: DO NOT use any emojis in this text, plain text only.",
  "tension_level": 6,
  "red_flags_in_this_scene": ["red flag 1", "red flag 2"],
  "money_lost": 0,
  "choice_A": "THE DANGEROUS NEXT CHOICE",
  "choice_B": "THE UNCERTAIN/RISKY NEXT CHOICE",
  "choice_C": "THE SAFE NEXT CHOICE"
}}
"""
        user_prompt = (
            f"Scenario: {scenario_idea}. "
            f"The user JUST CHOSE TO: '{user_choice}'. "
            f"Write the DIRECT visual and narrative consequence of this specific action, and provide exactly 3 logical next choices.{memory_context}"
        )
...
Example: "It's all gone. My entire savings account is empty. What have I done?"
DO NOT use the narrator voice here. The final debrief will use the narrator voice.
...
JSON structure:
{
  ...
  "scene_dialogue": "The victim's first-person panic or realization (max 30 words). USE 'I' AND 'MY'. STRICT RULE: DO NOT use any emojis in this text, plain text only.",
  "tension_level": 10,
...
Example: "You successfully identified the red flags and protected your identity. Stay vigilant out there."
...
{{
  ...
  "scene_dialogue": "The narrator's second-person summary and educational takeaway (max 30 words). USE 'YOU'. STRICT RULE: DO NOT use any emojis in this text, plain text only.",
  "tension_level": 8,
